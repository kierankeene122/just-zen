...
const ITEM_PROBES={
 ...
 'chat.google.com':()=>[...document.querySelectorAll('[role="listitem"][aria-label*="unread" i],[data-unread="true"]')].slice(0,8).map(el=>({title:(el.getAttribute('aria-label') || el.textContent).replace(/,?\s*\d+ unread.*$/i,'').trim().slice(0,80),sub:'unread',url:''})).filter(i=>i.title),
 'teams.microsoft.com':()=>[...document.querySelectorAll('[aria-label*="unread" i]')].slice(0,8).map(el=>({title:(el.getAttribute('aria-label') || '').replace(/unread.*$/i,'').trim().slice(0,80),sub:'unread',url:''})).filter(i=>i.title)
};
