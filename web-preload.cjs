...
// Notifications the page raises itself (Slack, WhatsApp, Telegram, Discord, Google Chat) are mirrored to the app with
// their title and body, so the Recent panel can show the message, not just a count. The wrapper lives in the page's
// world (installed from the main process); it posts a window message that this isolated world relays.
if(window===window.top){let lastNote='';window.addEventListener('message',e=>{if(e.source!==window || !e.data || e.data.__zen!=='notification')return;const title=String(e.data.title || '').slice(0,200),body=String(e.data.body || '').slice(0,500);const sig=title+'\0'+body;if(sig===lastNote)return;lastNote=sig;ipcRenderer.send('web-notification',{id:e.data.id,title,body});});ipcRenderer.on('web-notification-click',(_e,id)=>{try{window.postMessage({__zen:'notification-click',id},'*');}catch{}});}
// Sites that notify through a service worker never reach the wrapper; watch their message lists instead.
const MESSAGE_PROBES={
 'chat.google.com':()=>{const nodes=[...document.querySelectorAll('[data-message-id], [role="listitem"][data-id]')];const last=nodes[nodes.length-1];if(!last)return null;const sender=last.querySelector('[data-member-id], [data-hovercard-id], [role="heading"]')?.textContent || '';const text=(last.querySelector('[data-message-text], .Zc1Emd, [jsname="bgckF"]')?.textContent || last.textContent || '').trim();return text?{sender:sender.trim().slice(0,80),text:text.slice(0,300)}:null;},
