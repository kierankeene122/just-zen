...
if(window===window.top){let lastNote='';window.addEventListener('message',e=>{if(e.source!==window || !e.data || e.data.__zen!=='notification')return;const title=String(e.data.title || '').slice(0,200),body=String(e.data.body || '').slice(0,500);const sig=title+'\0'+body;if(sig===lastNote)return;lastNote=sig;ipcRenderer.send('web-notification',{title,body});});}
