...
The included local `.app` is still ad hoc signed, unnotarized, and has no live update feed. Current Rambox therefore remains operationally safer for ordinary web-app aggregation. Just Zen also has a different trust boundary because Claude can act on an explicitly granted local workspace. Claude Terminal has the same macOS-user authority as running Claude Code in Terminal.app; Just Zen does not grant it additional privilege.
