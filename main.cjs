...
function peekClose(){if(!peek)return;const {view}=peek;peek=null;try{win.contentView.removeChildView(view);}catch{}try{view.webContents.close();}catch{}send('peek-closed');}
function peekPlace(box){if(!peek)return;if(!box?.visible){peek.view.setVisible(false);peek.visible=false;return;}win.contentView.addChildView(peek.view);peek.view.setBounds(clipBounds(box));peek.view.setVisible(!locked);peek.visible=true;}
...
app.whenReady().then(async () => {
  ...
  handle('chat-send',async (prompt,meta)=>{if(!claudeRoot)throw Error('Choose a Claude workspace first');if(terminal)throw Error('Stop the terminal session before sending a chat message');if(chat.state.busy)throw Error('Wait for the current reply');const sandbox=await prepareClaudeSandbox({fs,root:claudeRoot,mode:config.claudePolicy || 'notes',userData:app.getPath('userData'),packageRoot:__dirname,nodePath:findNode(),claudePath:findClaude(),configDir:claudeConfigDir});chat.run(prompt,claudeRoot,sandbox.executable,{from:cleanFrom(meta?.from)}).catch(error=>send('notice',error.message));return true;});
