...
let layout={agentCollapsed:false,centreCollapsed:false,navCollapsed:true,navColumns:1,navDock:true},catalog=[],catalogPage=0,noteMode='preview';
...
async function applyLayout(next,persist=true){layout={...layout,...next};if(layout.centreCollapsed)layout.agentCollapsed=false;document.body.classList.toggle('agent-collapsed',layout.agentCollapsed);document.body.classList.toggle('centre-collapsed',layout.centreCollapsed);const dock=layout.navDock!==false;document.body.classList.toggle('nav-collapsed',layout.navCollapsed && !dock);document.body.classList.toggle('nav-two',layout.navColumns===2 && !dock);document.body.classList.toggle('nav-grey',Boolean(layout.navGrey));document.body.classList.toggle('nav-dock',layout.navDock!==false);dockMetrics();$('collapse-nav').textContent=layout.navCollapsed?'›':'‹';$('collapse-nav').setAttribute('aria-label',layout.navCollapsed?'Expand sidebar':'Collapse sidebar');$('agent-rail').classList.toggle('hidden',!layout.agentCollapsed);$('restore-centre').classList.toggle('hidden',!layout.centreCollapsed);placeViews();size();if(persist)await saveLayout();}
...
async function openGroupPopover(folder,row){closePopovers();const rect=row.getBoundingClientRect(),aside=document.querySelector('aside').getBoundingClientRect();if(layout.navDock!==false)await call('show-group-popover',{folderId:folder.id,left:Math.max(8,rect.left-120),top:rect.top-10,above:true});else await call('show-group-popover',{folderId:folder.id,left:aside.right+6,top:rect.top});}
...
function calendarBounds(){const surface=$('calendar-surface'),r=surface.getBoundingClientRect();call('calendar-bounds',{x:r.x,y:r.y,width:r.width,height:r.height,visible:document.body.classList.contains('calendar-open')}).catch(()=>{});}
$('calendar-rail').onclick=()=>{document.body.classList.add('calendar-open');requestAnimationFrame(()=>{calendarBounds();placeViews();size();});};
$('close-calendar').onclick=()=>{document.body.classList.remove('calendar-open');calendarBounds();requestAnimationFrame(()=>{placeViews();size();});};
$('calendar-reload').onclick=()=>attempt(()=>call('calendar-reload'));
window.hearth.on('calendar-hide',()=>{document.body.classList.remove('calendar-open');calendarBounds();});
new ResizeObserver(calendarBounds).observe($('calendar-surface'));
