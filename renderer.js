...
window.hearth.on('terminal-exit',code=>{running=false;const wasLogin=loginInProgress;loginInProgress=false;terminal.reset();if(wasLogin){applyMode('chat');attempt(()=>call('appearance',{mode:'chat'}));notice(code===0?'Signed in to Claude. You can chat now.':'Sign-in did not complete · exit '+code);return;}applyMode(mode);notice('Terminal session ended · exit '+code);});
