...
function deckLayout(){const cards=deckCards(),n=cards.length;const zone=$('deck-zone');const room=Math.max(120,zone.clientWidth-64-64);const fanned=document.querySelector('body>aside').classList.contains('fanned');
 const cw=Math.max(120,Math.min(200,Math.floor(room/Math.max(1,Math.min(n,3)))));document.body.style.setProperty('--card-w',cw+'px');
 const step=n>1?Math.min(fanned?cw+8:cw*.72,(room-cw)/(n-1)):0;
 cards.forEach((card,i)=>{const back=n-1-i;card.style.setProperty('--l',Math.round(64+i*step)+'px');card.style.setProperty('--dy',(-Math.min(back,10))+'px');card.style.setProperty('--sc',String(Math.max(.9,1-back*.008)));card.style.setProperty('--z',String(i+1));card.dataset.slot=String(i+1);card.title=card.dataset.name?card.dataset.name+' · ⌘'+Math.min(9,i+1):'';});
 const ghost=$('deck-ghost');const front=cards.find(c=>c.classList.contains('front'));if(front){ghost.classList.remove('hidden');ghost.style.setProperty('--l',front.style.getPropertyValue('--l'));}else ghost.classList.add('hidden');}
...
function deckSync(){const slot=page==='browser-page' && !layout.centreCollapsed?focusedSlot():null;const key=slot?.serviceKey || null;
 ...
 const companion=key?companionFor(key):null;
 for(const card of deckCards()){const mine=card.dataset.key===key;card.classList.toggle('front',mine);card.classList.toggle('companion',Boolean(companion) && card.dataset.key===companion);card.querySelector('.nav')?.setAttribute('aria-selected',String(mine));}
 deckLayout();}
function deckSelect(index){const card=deckCards()[index];const item=card?serviceOf(card.dataset.key):null;if(item)return openService(item);}
...
function deckCompanion(){const card=deckCards().find(c=>c.classList.contains('companion'));const item=card?serviceOf(card.dataset.key):null;if(item)return openService(item);notice('No companion yet. Switch between two apps a few times and the deck will learn the pair.');}
