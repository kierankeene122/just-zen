...
function closePeek(){if(!peekOpenState)return;peekOpenState=false;setTimeout(placeViews,60);$('peek').classList.remove('peek-in');setTimeout(()=>{$('peek').classList.add('hidden');},180);call('peek-close').catch(()=>{});}
window.hearth.on('peek-opened',({name,live})=>{peekOpenState=true;$('peek-tab').textContent=live?'Open here':'Open as tab';$('peek-title').textContent=(live?'Peek · ':'Quick look · ')+name;$('peek-url').textContent='';$('peek').classList.remove('hidden');requestAnimationFrame(()=>requestAnimationFrame(()=>{$('peek').classList.add('peek-in');setTimeout(()=>{peekBounds();placeViews();},200);}));if(page!=='browser-page')attempt(()=>show('browser-page'));});
...
window.hearth.on('peek-closed',()=>{if(!peekOpenState)return;peekOpenState=false;setTimeout(placeViews,60);$('peek').classList.remove('peek-in');setTimeout(()=>$('peek').classList.add('hidden'),180);});
