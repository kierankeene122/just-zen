...
let layout={agentCollapsed:false,centreCollapsed:false,navCollapsed:true,navColumns:1},catalog=[],catalogPage=0,noteMode='preview';
...
const tabLive=new Map(),asleep=new Set();let mutes={},zoomByApp={},recent=[];
...
window.hearth.on('recent-changed',list=>{recent=list || [];renderRecent();});
...
function sidebarMenu(x,y){showMenu([{label:layout.navCollapsed?'Expand sidebar':'Collapse sidebar to icons',run:()=>applyLayout({navCollapsed:!layout.navCollapsed})},'-',{label:'Icons in one column, scrolling',checked:layout.navColumns!==2,run:()=>applyLayout({navCollapsed:true,navColumns:1})},{label:'Icons in two columns',checked:layout.navColumns===2,run:()=>applyLayout({navCollapsed:true,navColumns:2})},'-',{label:'Greyscale icons',checked:Boolean(layout.navGrey),run:()=>applyLayout({navGrey:!layout.navGrey})}],x,y);}
