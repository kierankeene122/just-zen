...
async function start(kind){if(!claudeRoot && !await chooseClaude())return;await call('start-terminal',kind);running=true;await applyLayout({agentCollapsed:false});applyMode('terminal');requestAnimationFrame(size);terminal.focus();}
...
window.hearth.on('terminal-exit',code=>{running=false;applyMode(mode);terminal.reset();notice('Terminal session ended · exit '+code);});
