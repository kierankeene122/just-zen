...
async function show(next){if(next===page && !$(next).classList.contains('hidden'))return;await fadeOutPage();page=next;if(layout.centreCollapsed)await applyLayout({centreCollapsed:false});PAGES.forEach(id=>$(id).classList.toggle('hidden',id!==next));fadeInPage(next);$('home').classList.toggle('selected',next==='overview');$('security-nav').classList.toggle('selected',next==='security-page');$('brain-nav').classList.toggle('selected',next==='brain-page');placeViews();updateZoomControl();if(next==='security-page'){await renderSecurityStatus();await renderPasswords().catch(()=>{});}if(next!=='brain-page')brain.stop();}
