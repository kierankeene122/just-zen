...
function closePeek(){if(!peekOpenState)return;peekOpenState=false;hoverPeekKey=null;clearTimeout(hoverPeekTimer);setTimeout(placeViews,60);$('peek').classList.remove('peek-in');setTimeout(()=>{$('peek').classList.add('hidden');},180);call('peek-close').catch(()=>{});}
window.hearth.on('peek-opened',({name,live})=>{peekOpenState=true;if(!live)hoverPeekKey=null;$('peek-tab').textContent=live?'Open here':'Open as tab';$('peek-title').textContent=(live?'Peek · ':'Quick look · ')+name;$('peek-url').textContent='';$('peek').classList.remove('hidden');requestAnimationFrame(()=>requestAnimationFrame(()=>{$('peek').classList.add('peek-in');setTimeout(()=>{peekBounds();placeViews();},200);}));if(page!=='browser-page')attempt(()=>show('browser-page'));});
...
window.hearth.on('peek-closed',()=>{if(!peekOpenState)return;peekOpenState=false;hoverPeekKey=null;setTimeout(placeViews,60);$('peek').classList.remove('peek-in');setTimeout(()=>$('peek').classList.add('hidden'),180);});
...
let hoverPeekTimer=null,hoverPeekKey=null;
function armHoverPeek(row,key){if(layout.navDock===false)return;row.addEventListener('mouseenter',()=>{clearTimeout(hoverPeekTimer);hoverPeekTimer=setTimeout(()=>{if(page==='browser-page' && tiles.slots.slice(0,slotCount()).some(s=>s?.serviceKey===key))return;hoverPeekKey=key;attempt(()=>call('peek-app',key));},650);});row.addEventListener('mouseleave',()=>{clearTimeout(hoverPeekTimer);});}
document.addEventListener('mousedown',e=>{if(peekOpenState && hoverPeekKey && !e.target.closest('#peek') && !e.target.closest('.service-row,.service-group'))closePeek();},true);
function serviceRow(item){
 ...
 row.oncontextmenu=e=>{e.preventDefault();rowMenu(item,e.clientX,e.clientY);};armHoverPeek(row,key);
