...
function renderTiles(){
 ...
 for(let i=0;i<4;i++){
  ...
  if(!slot || !item || !group){
   ...
   const pick=document.createElement('select');pick.setAttribute('aria-label','App for this tile');pick.add(new Option('Choose an app…',''));for(const s of sidebarItems.filter(openable))pick.add(new Option((isBrowserItem(s)?(s.icon || '🌐')+' ':'')+s.name,s.id || s.url));
  ...
  if(browser){const label=el('button','tile-app');label.append(el('span','tile-emoji',item.icon || '🌐'),el('span',null,item.name));label.title=item.name;label.onclick=()=>{tiles.focus=i;};bar.append(label);}
...
function closePopovers(){$('ctx-menu').classList.add('hidden');$('ctx-menu').replaceChildren();call('hide-group-popover').catch(()=>{});if(popoverSuspended){popoverSuspended=false;resumeViews();}}
...
function showMenu(items,x,y){
 ...
 const w=menu.offsetWidth,h=menu.offsetHeight,aside=document.querySelector('aside').getBoundingClientRect();
...
document.addEventListener('mousedown',e=>{if(!e.target.closest('#ctx-menu'))closePopovers();},{capture:true});
...
function rowMenu(item,x,y){
 ...
 const browser=isBrowserItem(item);
 showMenu([{label:'Open',run:()=>openService(item)},{label:isSlack(item)?'Add another Slack workspace':'Open in a new tab',run:()=>newTab(key)},{label:'Open beside the current app',run:async()=>{if(tiles.mode==='1')tiles.mode='2h';await openInTile(key);}},'-',...(browser?[{label:'Edit name and icon…',run:()=>openBrowserEditor(item)},{label:'Clear this browser\'s cookies and logins',run:async()=>{if(!confirm('Clear cookies and website data for '+item.name+'? This signs its websites out.'))return;await call('clear-profile-data',{profile:'browser',key});tabs=await call('tabs');renderTiles();notice(item.name+' cleared.');}}]:[{label:'Sleep when hidden…',keep:true,run:()=>sleepMenu(key)}]),{label:'Move to group…',keep:true,run:()=>groupMenu(key)},'-',{label:'Remove '+item.name,run:async()=>{if(!confirm('Remove '+item.name+' from the sidebar?'))return;services(await call('remove-service',key));notice(item.name+' removed from the sidebar.');}}],x,y);
}
...
async function openGroupPopover(folder,row){closePopovers();const rect=row.getBoundingClientRect(),aside=document.querySelector('aside').getBoundingClientRect();await call('show-group-popover',{folderId:folder.id,left:aside.right+6,top:rect.top});}
...
function iconGrid(container,input,selected){if(!container.children.length){for(const icon of GROUP_ICONS){const b=el('button',null,icon);b.type='button';b.setAttribute('aria-pressed','false');b.onclick=()=>{for(const other of container.children)other.setAttribute('aria-pressed','false');b.setAttribute('aria-pressed','true');input.value=icon;};container.append(b);}}for(const b of container.children)b.setAttribute('aria-pressed',String(b.textContent===selected));}
let editingBrowser=null;
async function openBrowserEditor(item){editingBrowser=item;await openPicker('browser');}
function renderBrowserForm(){const editing=editingBrowser;$('browser-name').value=editing?editing.name:'';$('browser-icon').value=editing?editing.icon || '':'';iconGrid($('browser-icons'),$('browser-icon'),editing?editing.icon:'');$('browser-form').querySelector('.dialog-actions button').textContent=editing?'Save browser':'Add browser';$('browser-form').querySelector('p').textContent=editing?'Editing “'+editing.name+'”.':'A browser is a tab bar for any website, with its own logins and cookies. Keep one per client, project or side of your life.';}
function renderGroupForm(){const icons=$('group-icons');if(!icons.children.length){for(const icon of GROUP_ICONS){const b=el('button',null,icon);b.type='button';b.setAttribute('aria-pressed','false');b.onclick=()=>{for(const other of icons.children)other.setAttribute('aria-pressed','false');b.setAttribute('aria-pressed','true');$('group-icon').value=icon;};icons.append(b);}}
 const editing=editingGroup;$('group-name').value=editing?editing.name:'';$('group-icon').value=editing?editing.icon || '':'';for(const b of icons.children)b.setAttribute('aria-pressed',String(Boolean(editing && b.textContent===editing.icon)));$('group-form').querySelector('.dialog-actions button').textContent=editing?'Save group':'Create group';$('group-form').querySelector('p').textContent=editing?'Editing “'+editing.name+'”. Tick the apps that belong in it.':'A group is one tile in the sidebar that opens a visual menu of related apps.';
...
function staticPaletteItems(){
 const items=[];
 for(const item of sidebarItems)items.push({label:(isBrowserItem(item)?(item.icon || '🌐')+' ':'')+item.name,hint:isBrowserItem(item)?'Browser · tabs for any website':item.url?new URL(item.url).hostname.replace(/^www\./,''):'App',run:()=>openService(item)});
 for(const folder of serviceFolders)items.push({label:folder.icon+' '+folder.name,hint:'Group',run:()=>openGroupPopover(folder,document.querySelector(`.service-group[data-folder="${CSS.escape(folder.id)}"]`))});
 items.push({label:'New tab',hint:'In the current app · ⌘T',run:()=>{const slot=page==='browser-page'?focusedSlot():null;return slot?newTab(slot.serviceKey):openBrowser();}},{label:'One app in the centre',hint:'Layout',run:()=>setTilesMode('1')},{label:'Two apps side by side',hint:'Layout',run:()=>setTilesMode('2h')},{label:'Two apps stacked',hint:'Layout',run:()=>setTilesMode('2v')},{label:'Four apps in a grid',hint:'Layout',run:()=>setTilesMode('4')});
