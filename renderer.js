...
function closePopovers(){$('ctx-menu').classList.add('hidden');$('ctx-menu').replaceChildren();if(popoverSuspended){popoverSuspended=false;resumeViews();}}
...
function groupRow(folder,members){
 const row=document.createElement('div');row.className='service-group';row.dataset.folder=folder.id;row.draggable=true;if(!folder.collapsed)row.classList.add('open');
 ...
 row.ondragend=()=>{dragKey=null;document.querySelectorAll('.service-row,.service-group').forEach(r=>r.classList.remove('dragging','drop-target'));};
 const b=document.createElement('button');b.className='nav';b.title=folder.name+' · '+members.length+' app'+(members.length===1?'':'s')+(folder.collapsed?' · click to open':'');
 ...
 const chevron=document.createElement('small');chevron.className='group-chevron';chevron.textContent=folder.collapsed?'▸':'▾';
 b.append(icon,label,count,badge,chevron);b.onclick=()=>attempt(toggleGroup(folder));
 ...
 const head=document.createElement('div');head.className='group-head';head.append(grip,b,remove);row.append(head);
 if(!folder.collapsed){
  const grid=el('div','group-grid');
  for(const item of members){
   const key=item.id || item.url;const tile=el('button','group-tile');tile.type='button';const img=el('img','site-favicon');img.alt='';img.hidden=true;img.dataset.site=item.url || '';const fallback=el('span','site-fallback',item.name.slice(0,1).toUpperCase());
   const c=serviceBadges.get(key) || 0;const tb=el('small','service-badge',c>99?'99+':String(c || ''));tb.dataset.badgeKey=key;tb.hidden=!c;
   const out=el('b','mega-remove','×');out.title='Remove from this group';out.onclick=e=>{e.stopPropagation();attempt(async()=>{const state=await call('set-service-folder',{key,folderId:null});services(state.services,state.folders);notice(item.name+' is back in the sidebar.');});};
   tile.append(out,img,fallback,el('span',null,item.name),tb);tile.title=item.name+(item.url?' · '+hostOf(item.url):'');tile.onclick=()=>attempt(()=>openService(item));
   tile.draggable=true;tile.ondragstart=e=>{e.stopPropagation();dragKey=key;e.dataTransfer.setData('text/plain',key);e.dataTransfer.effectAllowed='move';};tile.ondragend=()=>{dragKey=null;document.querySelectorAll('.drop-target').forEach(x=>x.classList.remove('drop-target'));};
   tile.oncontextmenu=e=>{e.preventDefault();e.stopPropagation();showMenu([{label:'Open',run:()=>openService(item)},{label:'Open in a new tab',run:()=>newTab(key)},{label:'Remove from '+folder.name,run:async()=>{const state=await call('set-service-folder',{key,folderId:null});services(state.services,state.folders);notice(item.name+' is back in the sidebar.');}},'-',{label:'Sleep when hidden…',keep:true,run:()=>sleepMenu(key)},{label:'Move to group…',keep:true,run:()=>groupMenu(key)}],e.clientX,e.clientY);};
   call('favicon',key).then(icon=>paintIcon(img,icon)).catch(()=>{});grid.append(tile);
  }
  if(!members.length)grid.append(el('p','group-empty','Drag apps here, or right-click an app and choose “Move to group”.'));
  row.append(grid);
 }
 return row;
}
function toggleGroup(folder){return async()=>{const state=await call('toggle-service-folder',folder.id);services(state.services,state.folders);};}
function groupRowMenu(folder,x,y){showMenu([{label:folder.collapsed?'Open':'Close',run:toggleGroup(folder)},{label:'Rename…',run:()=>renameGroup(folder)},{label:'Change icon…',run:()=>changeGroupIcon(folder)},{label:'Add apps…',run:()=>openPicker('installed')},'-',{label:'Remove group',run:()=>removeGroup(folder)}],x,y);}
...
function orderedEntries(items,folders,order){const out=[],seen=new Set();for(const entry of order || []){if(seen.has(entry))continue;if(entry.startsWith('group:')?folders.some(f=>f.id===entry.slice(6)):items.some(i=>i.url && (i.id || i.url)===entry)){out.push(entry);seen.add(entry);}}for(const f of folders)if(!seen.has('group:'+f.id))out.push('group:'+f.id);for(const i of items){const k=i.id || i.url;if(i.url && !seen.has(k))out.push(k);}return out;}
...
function staticPaletteItems(){
 ...
 for(const folder of serviceFolders)items.push({label:folder.icon+' '+folder.name,hint:'Group',run:async()=>{if(layout.navCollapsed)await applyLayout({navCollapsed:false});if(folder.collapsed)await toggleGroup(folder)();}});
